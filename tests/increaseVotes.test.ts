...
import { IncreaseVotesUseCase } from "../src/application/useCases/polls/IncreaseVotesUseCase";

let pollRepository: InMemoryPollRepository;
let sut: IncreaseVotesUseCase;

describe("INCREASE VOTES", () => {
  beforeEach(async () => {
    pollRepository = new InMemoryPollRepository();
    sut = new IncreaseVotesUseCase(pollRepository);
  });

  it("Should be able to increase votes in a poll", async () => {
